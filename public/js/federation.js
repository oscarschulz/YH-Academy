...
/* PATCH: YHU instant Federation dashboard child tabs v1 */
function isFederationDashboardEmbeddedNavigationV1() {
...
function markFederationDashboardNonBlockingSyncV1(label = "") {
  const cleanLabel = String(label || "Syncing Federation...").trim() || "Syncing Federation...";
  ...
  [document.body].filter(Boolean).forEach((body) => {
    ...
    window.clearTimeout(window.__yhFederationNonBlockingTabSyncTimerV1);
    window.__yhFederationNonBlockingTabSyncTimerV1 = window.setTimeout(() => {
      body.removeAttribute("data-yh-tab-syncing");
      body.removeAttribute("data-yh-tab-sync-label");
    }, 900);
  });
}
/* END PATCH: YHU instant Federation dashboard child tabs v1 */
